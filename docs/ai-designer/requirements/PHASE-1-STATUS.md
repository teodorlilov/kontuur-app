...
| 2.2 image core | **Deterministic core done** — `src/lib/brand-kit/extract/kmeans.ts` (deterministic weighted k-means palette), `image-palette.ts` (`paletteToRoles` → 5 roles by luminance+saturation). Unit-tested. **Remaining (deploy-gated): image decode → pixels, Claude vision (mood/subjects/font category), the app route.** |
