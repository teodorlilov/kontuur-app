...
export interface VisualReviewData {
  ...
  /** Generated starter vector marks (sanitised SVG source) — shown as a "Brand marks" strip. */
 ...
 *  `<img>` (no script execution), so it renders crisply at any size. */
function BrandMarks({ svgs }: { svgs: string[] }) {
  return (
    <div>
      <div
        style={{
  ...
  return (
    <ReviewCard id="visual" title="Visual system" icon={<Palette size={10} color="var(--color-muted)" />}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        {visual.designVectors && visual.designVectors.length > 0 && <BrandMarks svgs={visual.designVectors} />}
