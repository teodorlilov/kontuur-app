...
export function RailBox({ title, tone = 'default', className, children }: RailBoxProps) {
  return (
    ...
    >
      <h4 className="mb-2.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-text3">
        {title}
      </h4>
      {children}
    </div>
...
export function RailText({ children }: { children: ReactNode }) {
  return <p className="text-[12.5px] leading-[1.55] text-text2 [&+&]:mt-2">{children}</p>
}
