...
describe('coverCrop', () => {
  ...
  it('a default transform is identical to no transform', () => {
    ...
    const withDefault = coverCrop(1024, 1024, CANVAS_WIDTH, CANVAS_HEIGHT, {
      zoom: 1,
      offsetX: 0.5,
      offsetY: 0.5,
    })
  ...
  it('offsets 0 and 1 pin the window to the source edges', () => {
    const left = coverCrop(1024, 1024, CANVAS_WIDTH, CANVAS_HEIGHT, {
      zoom: 1,
      offsetX: 0,
      offsetY: 0,
    })
    expect(left.cropX).toBe(0)
    const right = coverCrop(1024, 1024, CANVAS_WIDTH, CANVAS_HEIGHT, {
      zoom: 1,
      offsetX: 1,
      offsetY: 1,
    })
  ...
  it('an offset has no effect on the axis with zero slack', () => {
    ...
    const shifted = coverCrop(1088, 1360, CANVAS_WIDTH, CANVAS_HEIGHT, {
      zoom: 1,
      offsetX: 1,
      offsetY: 0,
    })
