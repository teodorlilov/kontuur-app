...
/**
 * The action boundary has to be at least as strict as the URL one. `resolvePeriod` clamps a
 * custom range to CUSTOM_MAX_DAYS; `fillPeriodData` turns whatever window it is handed into
 * Graph calls, so a wider window reaching the action directly spends the agency's quota on a
 * range no UI can produce.
 */
