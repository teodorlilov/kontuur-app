...
import { parseSitemapXml, parseSitemapFromRobotsTxt, discoverSitemapUrls, fetchSingleSitemap } from '../discover-sitemap'
...
describe('parseSitemapXml', () => {
  ...
  it('caps URLs at 5000', () => {
    const entries = Array.from({ length: 5100 }, (_, i) =>
...
describe('discoverSitemapUrls', () => {
  ...
  it('discovers via robots.txt Sitemap directive', async () => {
    mockFetch
      ...
      .mockImplementationOnce(() => mockResponse(`<urlset>
        <url><loc>https://example.com/page-1</loc></url>
        <url><loc>https://example.com/page-2</loc></url>
      </urlset>`))
  ...
  it('returns sub-sitemap refs instead of auto-following them', async () => {
    mockFetch
      .mockImplementationOnce(() => mock404()) // robots.txt
      .mockImplementationOnce(() => mockResponse(`<sitemapindex>
        <sitemap><loc>https://example.com/sitemap-posts.xml</loc></sitemap>
        <sitemap><loc>https://example.com/sitemap-pages.xml</loc></sitemap>
      </sitemapindex>`)) // /sitemap.xml (index)
...
describe('fetchSingleSitemap', () => {
  ...
  it('returns content URLs from a flat sitemap', async () => {
    mockFetch.mockImplementationOnce(() => mockResponse(`<urlset>
