...
import {
  ...
  buildEmptyDraft,
  seedBusinessName,
} from '@/features/onboarding/lib/build-draft'
import {
  ANSWERABLE_FIELDS,
  blockingFields,
  buildCreateInput,
...
interface ClientSetupFlowProps {
  /** A solo workspace on its first run — mode solo and no client yet — rather than an agency adding a client. */
  isSolo: boolean
  /** The name typed at signup (`agencies.name`); seeds the draft in solo, empty otherwise. */
  businessName: string
}

/**
 * The add-a-client flow, and the set-up-your-business flow: one site read into an editable draft
 * sheet, saved through `createClient`, then the sources stepper and the success screen. Owns every
 * piece of draft state; the (onboarding) page is a thin server wrapper that reads the workspace.
 *
...
export function ClientSetupFlow({ isSolo, businessName }: ClientSetupFlowProps) {
  ...
  async function handleAnalyze() {
    ...
    const result = seedBusinessName(buildDraftFromAnalysis(analysis), businessName)
  ...
  function startManual() {
    const result = seedBusinessName(
      { draft: buildEmptyDraft(), provenance: {}, unanswered: [] },
      businessName
    )
    setDraft(result.draft)
    setProvenance(result.provenance)
  ...
  async function handleSave() {
    ...
    toast.success(isSolo ? 'Profile saved' : 'Client saved')
  ...
  return (
    <>
      <OnboardingShell wide={showDraft} onCancel={isSolo ? undefined : handleCancel}>
        {showDraft ? (
          <DraftSheet
            draft={draft}
      ...
      <DraftSaveBar
        ...
        onDiscard={isSolo ? undefined : handleCancel}
