...
- **Copy-change staleness:** wizard rewrites auto-RE-compose via `applyCopyToDoc` (hand-edited
  layers keep their wording via `textOverridden`; AI art is never re-rolled). Review/calendar
  copy edits show a one-line "text on visuals may be outdated" nudge (`nudgeStaleBakedText`);
  auto-refresh there is a fast-follow (TECH-DEBT 2.5).
...
shortcuts suspended while the inline-edit textarea is focused.
...
incl. `captionHook` + `applyCopyToDoc`, autofit, cover-crop, font-library incl. filter + both
style pairings resolve to Cyrillic-safe families), `lib/visual/__tests__/fal-size`
