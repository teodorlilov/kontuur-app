...
const setBackdropOpacity = (opacity: number) => (current: CanvasDoc) => ({
  ...current,
  backdrop: { ...current.backdrop, opacity },
})

describe('commitHistory', () => {
  it('records a changed doc as one undo step', () => {
    const history = commitHistory(initHistory(doc()), setBackdropOpacity(0.7))
    expect(history.past).toHaveLength(1)
    expect(history.present?.backdrop.opacity).toBe(0.7)
  })
  ...
  it('folds commits sharing a coalesce key inside the window into one step', () => {
    ...
    history = commitHistory(history, setBackdropOpacity(0.5), {
      coalesceKey: 'backdrop',
      now: 1000,
    })
    history = commitHistory(history, setBackdropOpacity(0.6), {
      coalesceKey: 'backdrop',
      now: 1200,
    })
    history = commitHistory(history, setBackdropOpacity(0.7), {
      coalesceKey: 'backdrop',
      now: 1400,
    })
    expect(history.past).toHaveLength(1)
    expect(history.present?.backdrop.opacity).toBe(0.7)
    expect(undoHistory(history).present?.backdrop.opacity).toBe(0.4)
  })
  ...
  it('starts a new step when the coalesce key changes', () => {
    ...
    history = commitHistory(history, setBackdropOpacity(0.6), { coalesceKey: 'zoom', now: 1100 })
    expect(history.past).toHaveLength(2)
  })

  it('does not fold unkeyed commits', () => {
    let history = initHistory(doc())
    history = commitHistory(history, setBackdropOpacity(0.5), { now: 1000 })
    history = commitHistory(history, setBackdropOpacity(0.6), { now: 1010 })
    expect(history.past).toHaveLength(2)
  })

  it('clears the redo stack', () => {
    let history = commitHistory(initHistory(doc()), setBackdropOpacity(0.7))
    history = undoHistory(history)
    expect(history.future).toHaveLength(1)
    history = commitHistory(history, setBackdropOpacity(0.2))
  ...
  it('caps the undo stack', () => {
    ...
    for (let step = 1; step <= UNDO_CAP + 10; step += 1) {
      history = commitHistory(history, setBackdropOpacity(step / 100))
    }
...
describe('undoHistory / redoHistory', () => {
  it('round-trips a commit', () => {
    const start = initHistory(doc())
    const committed = commitHistory(start, setBackdropOpacity(0.9))
    const undone = undoHistory(committed)
    expect(undone.present).toBe(start.present)
    expect(redoHistory(undone).present?.backdrop.opacity).toBe(0.9)
  })
