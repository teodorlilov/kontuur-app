...
async function connectInstagram(
  ...
  const igUser = await fetchInstagramUser(longLivedToken)
...
/**
 * Where a failed callback lands before a client is even established — the roster, with the
 * reason in the query. Spelled once: five early exits built this URL by hand, each a chance
 * to encode or name the parameter differently.
 */
function clientsError(reason: string): NextResponse {
  return NextResponse.redirect(
...
export async function GET(request: NextRequest) {
  ...
  if (errorParam) {
    return clientsError(searchParams.get('error_reason') ?? errorParam)
  }

  if (!code || !state) return clientsError('missing_params')

  // State is HMAC-signed by /api/meta/connect — reject anything we didn't issue
  const decoded = decodeOAuthState(state)
  if (!decoded) return clientsError('invalid_state')
  ...
  if (!auth.ok) return clientsError('not_authenticated')
  const owned = await verifyClientOwnership(auth.supabase, clientId, auth.agencyId)
  if (!owned) return clientsError('not_authorized')
  ...
  try {
    ...
    // The one catch covers both networks' branches — name the one that actually failed.
    console.error(`[meta/callback] ${decoded.platform} OAuth error:`, message)
    return NextResponse.redirect(
